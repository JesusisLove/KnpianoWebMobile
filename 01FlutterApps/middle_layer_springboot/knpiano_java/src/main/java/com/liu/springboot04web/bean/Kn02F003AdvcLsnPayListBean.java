...
protected String    lessonId;
    ...
    /* 监视数据正常异常状态 ：data_status
        在课程费用支付信息查询结果中，增加数据状态判断字段（data_status），用于标识每条记录的数据状态（正常数据/异常数据）。
        判断规则

        当 advc_flg = 1 时：直接标记为"正常数据"
        当 advc_flg = 0 时：需要进一步验证
        检查是否存在相同学生ID（stu_id）、相同科目ID（subject_id）、相同月份（schedual_date前7位）且扫码日期（scanqr_date）不为空的记录
        如果存在：表示预支付时的lesson_id没有签到，相应的该月份其他的lesson_id反而签到了，表明预支付时的排课学生没有来上的这节课，也被统计为已经上课完了的课了，这是在“撒谎”，所以要标记为"异常数据"
        如果不存在：表示这月分，学生还是真的没有来上课，因为所有排好的课程都是处于未签到的状态，是正常数据，所以，标记为"正常数据"

        数据状态的解释：
            0:表示该数据的逻辑状态是正常数据的状态 
