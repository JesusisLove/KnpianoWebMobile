...
@Repository
public class Kn03D003BnkDao implements InterfaceKnPianoDao {

    @Autowired
    private Kn03D003BnkMapper knBnk001Mapper;

    // 获取所有银行信息
    public List<Kn03D003BnkBean> getInfoList() {
        return knBnk001Mapper.getInfoList();
    }

    /* 画面检索 检索功能追加  开始 */ 
    // 获取所有符合查询条件的银行信息
    public List<Kn03D003BnkBean> searchBanks(Map<String, Object> params) {
        return knBnk001Mapper.searchBanks(params);
    }
    /* 画面检索 检索功能追加  结束 */ 

    // 根据ID获取银行信息
    public Kn03D003BnkBean getInfoById(String id) {
         Kn03D003BnkBean knBnk001Bean = knBnk001Mapper.getInfoById(id);
         return knBnk001Bean;
    }

    // 保存银行信息（新建或更新）
    public void save(Kn03D003BnkBean knBnk001Bean) {
    ...
    // 新增银行信息
    private void insert(Kn03D003BnkBean knBnk001Bean) {
        knBnk001Mapper.insertInfo(knBnk001Bean);
    }

    // 更新银行信息
    private void update(Kn03D003BnkBean knBnk001Bean) {
