...
import com.liu.springboot04web.dao.KnLsnPay001Dao;

import java.util.Collection;
@Controller
public class KnLsnPay001Controller{

    @Autowired
    KnLsnPay001Dao knLsnPay001Dao;

    // 【課費支払管理】ボタンをクリックして，全ての情報を表示すること
    @GetMapping("/kn_lsn_pay_001_all")
    public String list(Model model) {
        Collection<KnLsnPay001Bean> collection = knLsnPay001Dao.getInfoList();
    ...
    public String excuteInfoAdd(KnLsnPay001Bean knLsnPay001Bean) {
    ...
    @GetMapping("/kn_lsn_pay_001/{lsnPayId}/{lsnFeeId}")
        public String toInfoEdit(@PathVariable("lsnPayId") String lsnPayId, 
                                 @PathVariable("lsnFeeId") String lsnFeeId, 
                                  Model model) {
        KnLsnPay001Bean knLsnPay001Bean = knLsnPay001Dao.getInfoById(lsnPayId, lsnFeeId);
    ...
    public String excuteInfoEdit(@ModelAttribute KnLsnPay001Bean knLsnPay001Bean) {
        knLsnPay001Dao.save(knLsnPay001Bean);
        return "redirect:/kn_lsn_pay_001_all";
    }
