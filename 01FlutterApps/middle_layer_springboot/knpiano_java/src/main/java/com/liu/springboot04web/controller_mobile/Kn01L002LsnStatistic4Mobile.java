...
public class Kn01L002LsnStatistic4Mobile {
    ...
    @Autowired
    // 获取已经上课完了的课程
    private Kn02F002FeeDao kn01L002LsnScanedDao;
    ...
    public ResponseEntity<List<Kn02F002FeeBean>> getInfoStuLsnList(@PathVariable("stuId") String stuId,
        ...
        // 获取当前正在上课的所有学生的排课信息
        List<Kn02F002FeeBean> collection = kn01L002LsnScanedDao.getInfoLsnStatisticList(stuId, Integer.toString(year));
    ...
    public ResponseEntity<List<Kn01L002LsnBean>> getUnScanSQDateLsnInfoByYear(@PathVariable("stuId") String stuId,
                                                                              @PathVariable("selectedYear") Integer year) {
        // 获取当前已经排课了但是还没有上课的记录
        List<Kn01L002LsnBean> collection = kn01L002LsnUnScanedDao.getUnScanSQDateLsnInfoByYear(stuId, Integer.toString(year));
        return ResponseEntity.ok(collection);
    }
}
